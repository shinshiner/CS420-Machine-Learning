...
The pseudo-codes of my algorithm can refer to \ref{alg:kmean_vs_GMM}.
...
\begin{algorithm}[H]
	...
	\While{the convergence criterion of parameters or log likelihood is not satisfied}{
		...
		\begin{center}
			$\omega \leftarrow \dfrac{\pi_{k}\mathcal{N}(x_{n}|\mu_{k}, \Sigma_{k})}{\sum\limits_{j=1}^{K}\pi_{j}\mathcal{N}(x_{n}|\mu_{j}, \Sigma_{j})}$
		\end{center}
		...
		\begin{center}
			$N_{k} \leftarrow \sum\limits_{n=1}^{N}\gamma(z_{nk})$
		\end{center}
